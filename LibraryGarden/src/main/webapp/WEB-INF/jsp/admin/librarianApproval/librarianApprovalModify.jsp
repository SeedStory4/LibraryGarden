...
<body>
		...
		<div class="inner p-0">
			...
			<section class="section draft-section m-0 p-0">
				<div class="draft-header">
					<div class="section-title draft-title m-0">기안 수정</div>
				...
				<form name="frm">
					...
					<div class="draft-content">
						...
						<div class="draft-info">
							...
							<p>
								<span class="info-title">● 출판사(출판일)</span> <span class="info-content publisher">${requestScope.bv.publisher}(${fn:replace(requestScope.bv.publishedYear, '-', '.')})</span>
							</p>
							<p>
								<span class="info-title">● 전체쪽수</span> <span class="info-content totalPages">${requestScope.bv.totalPages}쪽</span>
							</p>
							<p>
								<span class="info-title">● ISBN</span> <span class="info-content isbn">${requestScope.bv.isbn}</span>
							</p>
							<p>
								<span class="info-title">● 서적정보</span> <span class="info-content info">${requestScope.bv.sizeWidth}mm * ${requestScope.bv.sizeHeight}mm / ${requestScope.bv.weight}g / ${requestScope.bv.category}</span>
							</p>
							<p>
								<span class="info-title">● 정가</span> <span class="info-content price">${requestScope.bv.price}</span>
							</p>
						</div>
					</div>
					...
					<div class="draft-actions-mt">
						<button type="button" class="draft-btn-small btn-submit" onClick="check()">수정</button>						
	...
	function check() {
		...
		let ans = confirm("수정하시겠습니까?");
	  	  if (ans == true) {
			  fm.action="${pageContext.request.contextPath}/admin/librarianApproval/${requestScope.aidx}/librarianApprovalModifyAction.do";
			  fm.method="post";
			  fm.submit();
		}
