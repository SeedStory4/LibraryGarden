...
<body class="custom-page">
	...
	<div class="wrapper">
		<section class="section p-0">
			<h2 class="section-title m-0 normal relative">결재관리 목록<a class="btn btn-primary absolute" href="${pageContext.request.contextPath}/admin/approval/approvalWrite.do">기안등록</a></h2>
			...
			<div class="contents">
				...
				<div class="book-list pt-0">
					<form action="${pageContext.request.contextPath}/admin/approval/approvalList.do">
					...
					<ul class="tab flex gap-3">
						<li class="shadow <c:if test="${empty requestScope.filter}">on</c:if>"><a href="${pageContext.request.contextPath}/admin/approval/approvalList.do?${queryParam}">전체</a></li>
						<li class="shadow <c:if test="${requestScope.filter eq '대기'}">on</c:if>"><a href="${pageContext.request.contextPath}/admin/approval/approvalList.do?status=대기&${queryParam}">대기</a></li>
						<li class="shadow <c:if test="${requestScope.filter eq '승인'}">on</c:if>"><a href="${pageContext.request.contextPath}/admin/approval/approvalList.do?status=승인&${queryParam}">승인</a></li>
						<li class="shadow <c:if test="${requestScope.filter eq '반려'}">on</c:if>"><a href="${pageContext.request.contextPath}/admin/approval/approvalList.do?status=반려&${queryParam}">반려</a></li>
					</ul>
					<div class="table">
						<table>
							<colgroup>
							...
							<tbody>
								...
								<tr>
									...
									<td><a href="${pageContext.request.contextPath}/admin/approval/${ad.aidx}/approvalDetail.do">${ad.title}</a></td>
									<td>${ad.author}</td>
									<td>${ad.publisher}</td>
						...
						<ul class="paging flex w-270 justify-center">
							...
							<li>
					          <a href="${pageContext.request.contextPath}/admin/approval/approvalList.do?page=${requestScope.pm.startPage - 1}&${queryParam}" aria-label="Previous">◀</a>
					        </li>
							</c:if> 
							
					        <c:forEach var="i" begin="${requestScope.pm.startPage}" end="${requestScope.pm.endPage}" step="1">
					        <li><a class="<c:if test="${i == requestScope.pm.scri.page}">on</c:if>" href="${pageContext.request.contextPath}/admin/approval/approvalList.do?page=${i}&${queryParam}">${i}</a></li>
					        </c:forEach>
					        
					        <c:if test="${requestScope.pm.next == true && requestScope.pm.endPage > 0}">
							<li class="page-item">
					          <a href="${pageContext.request.contextPath}/admin/approval/approvalList.do?page=${requestScope.pm.endPage + 1}&${queryParam}" aria-label="Next">▶</a>
					        </li>
							</c:if>
						</ul>
					</div>
